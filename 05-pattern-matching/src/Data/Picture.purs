...
showShape (Clipped p c w h) =
  "Clipped [center: " <> showPoint c <> ", width: " <> show w <> ", height: " <> show h <> ", picture: " <> showPicture p <> "]"

type Picture = Array Shape

showPicture :: Picture -> String
showPicture p = "Picture [" <> intercalate "\n," (map showShape p) <> "]"
...
shapeBounds (Clipped _ (Point { x, y }) w h) = Bounds
  { top:    y - h / 2.0
  , left:   x - w / 2.0
  , bottom: y + h / 2.0
  , right:  x + w / 2.0
...
area :: Shape -> Maybe Number
area (Circle _ r) = Just $ Math.pi * r * r
area (Rectangle _ w h) = Just $ w * h
area (Line _ _) = Just 0.0
area (Text _ _) = Just 0.0
area (Clipped _ _ _ _) = Nothing
